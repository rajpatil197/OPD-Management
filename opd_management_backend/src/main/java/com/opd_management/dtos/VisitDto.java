...
public class VisitDto {
   ...
   // @NotBlank(message = "Diagnosis field is required")
    ...
    @Pattern( regexp = "^[0-9]{2,3}/[0-9]{2,3}$",message = "BP must be in format ###/## (Example: 120/80)" )
    private String bp;

    @Pattern(regexp = "^[0-9]{2,3}$", message = "Pulse must be a valid number (Example: 75)" )
    private String pulse;

    @Pattern( regexp = "^[0-9]{2,3}%$", message = "Saturation must end with % (Example: 98%)")
    private String saturation;

    @Pattern(regexp = "^[0-9]{2,3}(\\.[0-9]{1,2})?(°?F|°?C)?$",message = "Temperature must be valid (Examples: 98.6°F, 37°C)")
    private String temperature;

    @Pattern(regexp = "^[0-9]{1,2}$",message = "Respiration rate must be a number (Example: 18)")
    private String respiration_rate;


    // ------------ Sugar Values --------------

    @Pattern(regexp = "^[0-9]{2,3}$", message = "Sugar must be numeric")
    private String sugar;

    @Pattern(regexp = "^[0-9]{2,3}$", message = "Fasting sugar must be numeric")
    private String fasting_sugar;

    @Pattern(regexp = "^[0-9]{2,3}$", message = "PP sugar must be numeric")
    ...
    @Size(max = 500, message = "Past history must be under 500 characters")
    private String past_history;

    @Size(max = 500, message = "Current medication must be under 500 characters")
    private String current_medication;

    @Size(max = 1000, message = "Additional notes must be under 1000 characters")
    ...
    @Pattern(regexp = "^(Yes|No|Mild|Moderate|Severe)?$", message = "Valid values: Yes, No, Mild, Moderate, Severe")
    ...
    @Size(max = 200, message = "CVS notes must be under 200 characters")
    private String cvs;

    @Size(max = 200, message = "RS notes must be under 200 characters")
    private String rs;

    @Size(max = 200, message = "PA notes must be under 200 characters")
    private String pa;

    @Size(max = 200, message = "CNS notes must be under 200 characters")
    private String cns;


    @Pattern(regexp = "^[0-9]{1,2}(\\.[0-9]{1,2})?$",message = "HB must be a valid number (Example: 13.5)")
    private String hb;


    @Size(max = 200, message = "ECG notes must be under 200 characters")
    private String ecg;
